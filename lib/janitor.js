...
Janitor.prototype.getMatchingLogGroups = function (options, callback) {
  ...
  if (options.include && !_.isRegExp(options.include)) {
    return callback(new Error(
      'If provided, options.include must be a RegExp instance.'
    ));
  }
    ...
    self.cwlClient.describeLogGroups({
      ...
      logGroups = logGroups.concat(_.filter(
        ...
        function (logGroup) {
          ...
          if (options.include) {
            keep = keep && options.include.test(logGroup.logGroupName);
          }
